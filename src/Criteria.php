...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'entity_table', 'entity_pk', 'field_name', 'ciphertext', 'meta', 'created_at', 'updated_at' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'entity_table', 'entity_pk', 'field_name' ];
    }

/** Columns allowed in ORDER BY (falls back to filterable() when empty). */
protected function sortable(): array
{
    return [ 'id', 'entity_table', 'entity_pk', 'field_name', 'created_at', 'updated_at' ];
}
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\EncryptedFields\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\EncryptedFields\Definitions::softDeleteColumn();
