 ...
 */
final class EncryptedFieldsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private EncryptedFieldRepository $encryptedFieldRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'encrypted_fields' => $this->encryptedFieldRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\EncryptedFields\ModuleException("Record {$idStr} not found for locking.");
            }
