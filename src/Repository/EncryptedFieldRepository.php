...
namespace BlackCat\Database\Packages\EncryptedFields\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\EncryptedFields\Definitions;
use BlackCat\Database\Packages\EncryptedFields\Criteria;
use BlackCat\Database\Packages\EncryptedFields\Dto\EncryptedFieldDto as Dto;
use BlackCat\Database\Packages\EncryptedFields\Mapper\EncryptedFieldDtoMapper as RowMapper;
...
final class EncryptedFieldRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\EncryptedFields\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'ciphertext', 'meta', 'updated_at' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'entity_table', 'entity_pk', 'field_name' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\EncryptedFields\Definitions::table());
            return $bulk->upsertMany($rows, [ 'entity_table', 'entity_pk', 'field_name' ], [ 'ciphertext', 'meta', 'updated_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EncryptedFields\Dto\EncryptedFieldDto|null */
    public function getByEntityTableAndEntityPkAndFieldName(string $entityTable, string $entityPk, string $fieldName, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptedFields\Dto\EncryptedFieldDto|null {
        $row = $this->getByUnique([ 'entity_table' => $entityTable, 'entity_pk' => $entityPk, 'field_name' => $fieldName ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\EncryptedFields\Mapper\EncryptedFieldDtoMapper::fromRow($row);
    }
    public function existsByEntityTableAndEntityPkAndFieldName(string $entityTable, string $entityPk, string $fieldName): bool {
        $where = 't.' . Ident::q($this->db, 'entity_table') . ' = :uniq_entity_table' . ' AND ' . 't.' . Ident::q($this->db, 'entity_pk') . ' = :uniq_entity_pk' . ' AND ' . 't.' . Ident::q($this->db, 'field_name') . ' = :uniq_field_name';
        return $this->exists($where, [ 'uniq_entity_table' => $entityTable, 'uniq_entity_pk' => $entityPk, 'uniq_field_name' => $fieldName ]);
    }
    /** @return int|string|null */
    public function getIdByEntityTableAndEntityPkAndFieldName(string $entityTable, string $entityPk, string $fieldName) {
        $row = $this->getByEntityTableAndEntityPkAndFieldName($entityTable, $entityPk, $fieldName);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EncryptedFields\Dto\EncryptedFieldDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptedFields\Dto\EncryptedFieldDto|null {
