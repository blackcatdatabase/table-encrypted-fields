...
use BlackCat\Database\Packages\EncryptedFields\Repository\EncryptedFieldRepositoryInterface;
...
class EncryptedFieldRepository implements EncryptedFieldRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'entity_table', 'entity_pk', 'field_name' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\EncryptedFields\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'ciphertext', 'meta', 'updated_at' ]);
          return count($rows);
        }
    ...
    public function getByEntityTableAndEntityPkAndFieldName(string $entityTable, string $entityPk, string $fieldName, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptedFields\Dto\EncryptedFieldDto|null {
        return $this->getByUnique([ 'entity_table' => $entityTable, 'entity_pk' => $entityPk, 'field_name' => $fieldName ], $asDto);
    }
    ...
    public function getIdByEntityTableAndEntityPkAndFieldName(string $entityTable, string $entityPk, string $fieldName) {
        $row = $this->getByEntityTableAndEntityPkAndFieldName($entityTable, $entityPk, $fieldName, false);
